...
namespace ChatBot.Bot.Plugins.LostRPG.DialogueSystem.Dialogue
{
    public class CreateUser : Dialogue
    {
        ...
        {
            SystemController.Instance.Respond(null, $"First off... what would you like to be known as? This is what you'll be called by and how others will call you. " +
        ...
        public void Two(string args)
        {
            ...
            SystemController.Instance.Respond(null, $"Okay, great~ Welcome to LostRPG, {Card.Alias}!" +
                $"\\n\\n" +
        ...
        public void Three(string args)
        {
            ...
            if (magic.Any(x => x.Name.ToLowerInvariant().Equals(args)))
            {
                ...
                SystemController.Instance.Respond(null, $"Great choice, {Card.Alias}. You're now a Mage that specializes in [color={tm.Color}][b]{tm.Name}[/b][/color] magic! " +
                ...
                Card.MagicData.PrimaryMagic = tm;
                DataDb.Instance.AddNewUser(Card);
                return;
            }
            else
            {
                SystemController.Instance.Respond(null, $"Sorry, I didn't understand... that doesn't seem to be a valid type of magic. Try again by typing Yes, No, Repeat, or Cancel.", Owner);
                CurrentStep = 2;
                BackingUp = true;
                return;
            }
